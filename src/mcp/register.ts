...
- {"op":"sdfDistance","path?":"/abs/surface.stl","part?":"Skin","sign?":"pseudonormal|winding|none","band?":0,"output?":"SDF_DISTANCE"} — the signed distance from every node to a surface, as a new nodal field (default SDF_DISTANCE). The surface is either an EXTERNAL mesh read from "path", or "part" — a SubModelPart already in THIS mesh (its subtree included, extracted the same way mesh_extract_submodelpart would) — no second file needed; exactly one of "path"/"part" must be given, both together is rejected. NEGATIVE IS INSIDE, and the surface must be closed for the sign to mean anything ("none" gives unsigned distance for an open one). The purest oracle here: only the surface crosses into wasm, the query points are our own coordinates, and one double comes back per node in order — so nothing about this mesh can change but the added field. Pairs directly with levelset: run sdfDistance, then {"op":"levelset","variable":"SDF_DISTANCE"} to CUT this mesh along that surface. "winding" is slower but tolerant of small holes; band computes exact values only within that distance and clamps beyond it. An unreadable path, or a part name not found, is a noop with a message, never a failure
