...
**Re-measured on 2026-09-22 against 15.4.0** (roadmap item 3's Tier 0 bump). No admitted reader's classification changed. Five new reader keys joined the routing tables in this bump (`frd`, `lsdyna`, `pcd`, `vtkhdf`, `xyz`); their audit results are below. `frd` and `vtkhdf` are options-aware but neither is admitted: `frd` fails the `fellBackToFullRead: false` bar despite having genuine step selection (see its own row), and `vtkhdf`'s selection behaviour is unmeasured — no JS-reachable stepped writer exists to build a genuine multi-step fixture from (`VtkhdfTimeSeriesWriter` is C++-only; see meshio.ts's sequence-export bullet). Both are left as named candidates for the still-pending remainder of roadmap item 3, not decided here. `lsdyna`/`pcd`/`xyz` report no options awareness at all and are not candidates.
...
| Options-aware, unmeasured — deferred | `vtkhdf` | Genuinely options-aware (>= 14.0.0 Steps), but no JS-reachable stepped writer exists here to build a multi-step fixture; classification needs one built via the C++/Python test harness or a real solver run before it can be admitted or excluded |
