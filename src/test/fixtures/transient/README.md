...
Every input below contains two samples, at times/indices 0 and 1. Scalar samples are `[10,20,30]` and `[40,50,60]`; EnSight changes the triangle's x extent from 1 to 2. All fixtures are synthetic, authored for this repository and covered by its license. None were downloaded from solver datasets.
...
The test pins the complete set of `MESHIO_READ_CANDIDATES` keys and probes `readerSupportsOptions` for each distinct reader. New keys or newly options-aware readers fail the audit test and require review. Options awareness alone does not mean time selection works (Gmsh is the counterexample).
