...
The verdict above is kept unchanged as the historical record, but a re-examination for roadmap item 18 found that two of its three structural findings are artifacts of how the spike loaded and measured the artifact, not properties of it. The full re-evaluation, with evidence, is [`doc/vtk-wasm-migration.md`](./vtk-wasm-migration.md).

- **"Every instance method returns a Promise" was caused by a missing method table.** The spike loaded the unpacked directory with `loadAsync({ url, urlIsGzip: false })`. In that mode the loader fetches `<url>/vtk-methods.json` to learn each method's C++ name and whether it may suspend; there was none, so its table was `null`, and with no table the loader wraps *every* method in an `async` function. `exec: "sync"` made no difference because it only chooses which file name to load. With a table generated from the tarball's own `types/*.json`, only the 13 methods flagged `maySuspend` (`Render`, pixel reads, `Start`/`Initialize`/`Frame`…) return a Promise; every getter and setter returns a plain value. The spike's own condition was reproduced exactly by serving the same binary without the table.
- **The ~110 µs per-call overhead was measured on calls that failed.** The benchmark passed a JS boolean (`i % 2 === 0`) to `setVisibility`; the session's invoker rejects booleans, so all 2,001 calls logged `No suitable overload of 'vtkProp::SetVisibility'` (those errors are in `g2-transfer-test.json`). A successful native call costs 4–6 µs.
- **`toJSTypedArray` was broken only by the same missing table**: it calls `getDataType()` without awaiting it, which is correct once that call is synchronous. Exact round trips were measured for every TypedArray kind.
- **The `'unsafe-eval'` finding stands for the published glue**, and is now resolved differently: both dynamic-code sites are mechanical Embind factories, replaced by a deterministic, differential-tested rewrite (`src/parser/render/vtkWasmGlue.ts`), after which the glue runs in a real VS Code webview under the shipped CSP plus only `'wasm-unsafe-eval'` and `connect-src`.
- **Smaller corrections.** `scripts/spike/` is tracked in git (the `.gitignore:10` rule is `out/`, which covers only the fetched binaries and results; the scripts stay out of the `.vsix` through `.vscodeignore`'s `scripts/**`). Line numbers cited above are stale (`webview/main.ts` is now 6,164 lines with 60 `renderWindow.render()` sites; the CSP lives at `src/webviewChrome.ts` `buildPreviewHtml`). The upstream `dist` branch history has since been rewritten, so commit `51747d0a` is reachable only by hash.
