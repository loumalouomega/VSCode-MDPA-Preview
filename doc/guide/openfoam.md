...
The reader takes the mesh and nothing else. Each of these is reported as a warning in the Information panel rather than silently omitted:
...
If `constant/polyMesh/boundary` is missing, the mesh loads with **no boundary faces at all** — that file is what defines them. `points`, `faces` and `owner` are required; without one the open fails naming the missing file.
