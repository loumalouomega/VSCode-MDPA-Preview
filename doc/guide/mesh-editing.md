...
Smooth, Reorder, Partition and Merge mesh run asynchronously — the same inline progress bar and play/stop cancel button as the MMG operations. The rest apply instantly. Every one of these joins the same undoable operation history and JSON recipe as the operations above, and is reachable from the `mesh_transform` tool on the [MCP server](./development), same as every other mesh operation, for scripting.
