...
**Partially delivered; the remainder is pending.** The Tier 0 bump to meshio++ 15.4.0 landed first (see the Research baseline above), then: DOLFIN/TetGen/EnSight export through the existing companion-aware writer contract, with a new `exportEligibility.ts` refusing an ineligible mesh cleanly before the write instead of an upstream RAISE partway through; MED's own mesh name, description and units surface through a new `MdpaModel.source` and MCP `mesh_info`'s conditional `source` section, and a lenient MED read's diagnostic now names the specific constructs it could not represent; FLAC3D's `zone:<name>:<slot>` group names read back clean and its structurally-2D FACE-section blocks read back as Conditions rather than Elements; a Gmsh export now names a SubModelPart it silently drops (node-only — no cell to carry a physical-group tag) instead of leaving no diagnostic at all; OpenFOAM `cellZones`/`faceZones`/`pointZones` are staged and read as named SubModelParts, and a recovered patch `type` needing no extra dictionary key (`wall`/`empty`/`symmetry`/`symmetryPlane`) survives a write instead of always defaulting to `patch`.

**Still pending:** additional in-file timelines beyond what the bump's own re-audit found (a native `.pvd` reader — see the Research baseline — plus reconsidering `frd`/`vtkhdf` once a genuine multi-step fixture exists for the latter), richer selective reads (meshio++'s new `piece`/`dropGhosts` options are not yet threaded through any tool), OpenFOAM field export (writing Elemental fields back as `0/<VAR>` volField files), OpenFOAM multi-region cases (reading every `constant/<region>/polyMesh` merged, not just the default), and OpenFOAM decomposed (`processor*/`) cases (upstream's own core already reconstructs these read-only per the Research baseline's evidence table — unverified here, since building a genuine decomposed fixture needs OpenFOAM's own `decomposePar`, unavailable in this environment).
