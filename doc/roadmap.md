...
**Shipped as of 4.6.0.** `mesh_probe` samples a nodal field along a polyline (gaps where the path leaves the mesh, optionally across every step of a series, CSV output) and slices, isosurfaces and threshold regions export as mesh files from the Clip dock and the Field panel; the interactive side shipped too: a **Probe line** button on the Inspect panel takes two picks the way Measure does, draws the line, and shows the distance-versus-value profile in a `seriesPanel`-style chart over one `meshAnalysis` round trip (`kind: "probe"`, the same `probeAlongPath` core `mesh_probe` calls) — the plot follows the timeline step (each frame re-requests; sequence-tagged replies drop stragglers during playback) and exports CSV. Time-series repetition was deliberately scoped to follow-the-frame, not a multi-file scan: the documented `meshAnalysis` contract is single-shot analyses of the currently loaded model, and a full over-steps scan would need the `fieldSeries` family instead of the one message kind the item named.

**Acceptance:** the plot's numbers equal `mesh_probe`'s for the same endpoints (asserted in `meshAnalysis.test.ts`), a path leaving the mesh breaks the line rather than bridging it, and the chart follows the timeline step. **MCP:** none beyond the shipped `mesh_probe`; the panel is UI-only.
...
**Shipped as of 4.5.0.** Selection sets over Elements/Conditions/Geometries (independent id spaces) with seeds — explicit picks (Ctrl+click), box select, a lasso (click-to-place polygon closed on the first point or Enter), a SubModelPart, a field window (the Threshold cells), a meshQuality metric or a property id — refresh **by definition** against every new model, which is the acceptance's "survives applicable edits through correspondence": predicates follow the timeline and the edits, explicit picks keep exactly the surviving ids and prune the rest. `createSubModelPartFromSelection` (node closure rides with the entities, so the parent/child subset rule holds), `deleteEntities` (the complement of the selection export, run through the same `restrictToCells` machinery — conditions on surviving ground stay, constraints vanish with their nodes), selection export, block-granularity Isolate/Hide/Restore, and a Properties editor (shared-edit vs clone-and-reassign, beam `CROSS_AREA`, `assignProperty`, delete guarded while referenced) are wired as ordinary undoable ops, keep the exact original ids through every applicable edit, and ride the model-emitted MDPA writer — plus `mesh_select` + the property ops inside `mesh_transform` as the MCP half.
