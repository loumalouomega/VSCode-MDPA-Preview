...
**Shipped as of 4.5.0, minus the lasso.** Selection sets over Elements/Conditions/Geometries (independent id spaces) with seeds — explicit picks (Ctrl+click), box select, a SubModelPart, a field window (the Threshold cells), a meshQuality metric or a property id — refresh **by definition** against every new model, which is the acceptance's "survives applicable edits through correspondence": predicates follow the timeline and the edits, explicit picks keep exactly the surviving ids and prune the rest. `createSubModelPartFromSelection` (node closure rides with the entities, so the parent/child subset rule holds), selection export through `restrictToCells`, block-granularity Isolate/Hide/Restore, and a Properties editor (shared-edit vs clone-and-reassign, beam `CROSS_AREA`, `assignProperty`, delete guarded while referenced) are wired as ordinary undoable ops, keep the exact original ids through every applicable edit, and ride the model-emitted MDPA writer — plus `mesh_select` + the property ops inside `mesh_transform` as the MCP half. The lasso is deliberately not shipped (the box covers the region case; a polygon adds no selection semantic the box does not) — reintroduce only if a workflow actually needs freeform regions.
