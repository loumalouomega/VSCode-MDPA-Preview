...
**meshio++ is adopted at the locked 16.14.0 baseline.** Historical binding failures, missing side channels, format defects, and build omissions are no longer exclusions from this roadmap — `@meshioplusplus/wasm` is pinned at `^16.14.0`, the locked baseline for this integration, and the extension's packaged runtime already declares and uses it. What remains is connecting its capabilities to the extension end to end, preserving Kratos semantics, and verifying the resulting workflows — the items below, not the dependency adoption itself. Keeping that pin current as upstream releases is its own standing item, Tier 0.
...
Reviewed `/home/vicente/src/meshioplusplus` on **2026-09-25**, including its changelog through 16.14.0, `bindings/wasm/js_bindings.cpp`, and capability documentation. The extension declares `@meshioplusplus/wasm: ^16.14.0`. **This is the first bump to cross a major version** — 16 releases from the 15.4.0 baseline, and the measurement below is against the *published* artifact (downloaded and loaded side by side with the installed 15.4.0), not against the changelog's prose.

The WASM boundary is unchanged and that is the load-bearing finding: `git diff v15.4.0..HEAD` over `bindings/wasm/js_bindings.cpp` and `index.d.ts` is **additive only** — one new export (`tensorInvariants`), zero removals, zero signature changes. The v16.0.0 C++ ABI break (`CellType::Custom` moving 76→77 with `Triangle7` appended) is irrelevant to a prebuilt-WASM consumer, so this is a routing exercise, not a compatibility one. The one boundary *behaviour* change is `read_with_info`/`readMeshSelective` for `unv` now taking `ReadOptions`.

Live capability probe, 15.4.0 → 16.14.0: **54 → 76 readers, 57 → 66 writers, 21 → 37 options-aware readers**, no key removed, `hasCgnslib` still true. 16.13.0's `vtx`/`szplt` are **absent from the WASM build entirely** (they need optional ADIOS2/TecIO, which this build does not link), so they need no routing decision. The bundled `dist/meshio/` grows 17.42 MB → 22.47 MB, **+5.05 MB across both variants**.

**22 new reader keys.** Eleven are solver *results*: `abaqus_fil`, `ansys_rst`, `ansys_rst_cyclic`, `lsdyna_binout`, `lsdyna_d3plot`, `marc_t19`, `nastran_h5`, `nastran_op2`, `radioss_anim`, `radioss_th`, `xplt`. Seven carry a real extension and are routed (`.fil`, `.rst`/`.rth`, `.d3plot`, `.t19`, `.h5`, `.op2`, `.xplt`); the other four are found upstream by **file name, not extension**, so no extension this extension offers can reach them and they stay `inputFormat`-only. **None is admitted to the in-file timelines**: every one reports `fellBackToFullRead: true`, the `frd` bar. Eleven are structural CAE formats (`code_aster`, `elmer`, `febio`, `femap`, `libmesh`, `marc`, `mfem`, `mphbin`, `patran`, `radioss`, `z88`) and are deferred to a new roadmap item, each named with a reason. All nine new *writers* belong to that deferred set, so `MESHIO_WRITER_KEYS` gained nothing and every read-only key now lives in one named `MESHIO_READ_ONLY_KEYS` set.

Behaviour changes touching code this extension already runs, all re-measured rather than assumed: the Fluent `ansys` **writer** was rewritten (16.9.0, now emits Fluent's own face-based layout its own reader accepts — the round trip survives, the row-for-row connectivity guarantee does not, and the test now says which); `conservative_interpolate` **lost up to 0.7% of mass** between aligned meshes until 16.14.0 fixed it, which is now pinned positively; UNV's group entity types were **swapped** and its `hexahedron20`/`wedge15` mid-edge order corrected (15.6.0); the Nastran bulk deck is now read **natively** (16.2.0), so `.bdf`/`.fem`/`.nas` open at all where they previously needed a Python twin wasm does not have; MED gained `HEXA27`/`PENTA18` and `triangle7` became a real cell type (16.0.0, `wedge18` and `triangle7` are now mapped to drawable VTK types); COMSOL's quadratic node order was **corrected** (16.1.0); and a non-transient **multi-zone Tecplot file now reads all its zones** instead of silently discarding all but the first (15.5.0).

The transient audit was re-run against 16.14.0 (`src/test/fixtures/transient/README.md`): **no existing classification changed.** Two already-routed readers flipped options-awareness: `unv` (genuinely, and still not admitted) and `ansysinp` (which registers a `ReadOptions` overload for a format with no time axis — the sharpest case of the flag not being capability). `mcpTools.test.ts` now **refuses** any unrouted key left on the `UNEXAMINED_REASON` fallback, so the next release's new formats fail a test until each is routed or its reason is written down. `.github/workflows/meshio-watch.yml` reports a major-version lag on a schedule, because a caret range never crosses a major and Dependabot's minor/patch group structurally cannot propose it.
...
| The 16.x changelog records optional-dependency readers `vtx` (16.13.0, ADIOS2) and `szplt` (16.13.0, TecIO), which the published WASM build **does not link** — `hasAdios2`/`hasTecio` are absent and neither key appears in `availableFormats()` | Nothing to do now, and the reason is worth recording: a format gated on a vendor library this build cannot ship is a different decision from one that merely lacks a wrapper. Revisit only if a dependency-free or freely-redistributable path appears. |
| The 16.x changelog adds eleven structural CAE readers (item 15) whose upstream round-trip matrix and format pages are complete, against a codebase that has no representation for a starter deck's control cards, a partition directory, or a Z88 fixed-name dispatch | Route them deliberately rather than on arrival: two can only be retry candidates, one is a directory, and one is dispatched by literal filename. See item 15's per-format notes. |

The historical audits in `src/test/fixtures/transient/README.md` and `CLAUDE.md` remain evidence about the versions tested. During integration, replace obsolete expectations with positive capability tests and retain relevant regression fixtures. **The 16.14.0 audit was a live-WASM one** (both artifacts loaded and probed side by side, upstream's own result fixtures staged read-only); earlier entries in this section were source inspection only, and their version claims are evidence about those versions, not about the current pin. A VTK-wasm rendering-runtime evaluation (standalone-session boot, bulk data transfer, and a minimal end-to-end render, testing the shipped webview CSP and required changes) was run and closed on **2026-09-18** with a drop verdict — see [`doc/vtk-wasm-spike.md`](./vtk-wasm-spike.md) for the full findings, measurements, and reproduction steps. That verdict remains evidence about the tested artifact. Item 18 then resolved its blockers for a newer build and shipped VTK-wasm as an **experimental, opt-in** renderer; by decision (2026-09-26) vtk.js remains the default and is not being replaced.
...
**Standing work — last bumped 2026-09-25, 15.4.0 → 16.14.0, verified against the published artifact.** The extension declares `@meshioplusplus/wasm: ^16.14.0` and its lockfile resolves 16.14.0, which **is** the latest published release as of that date. This was the first bump to cross a major version (16 releases); its full classification is in the Research baseline above. The four buckets below are now backed by measurement, not by the changelog's prose: the binding diff is additive-only, two already-routed readers flipped options-awareness, one kernel fix (16.14.0's `conservative_interpolate` mass loss) removes a defect this extension would otherwise have inherited silently, and all 22 new reader keys are either routed or carry a recorded reason — with `mcpTools.test.ts` refusing the unexamined fallback so the next bump fails a test rather than a user.

The detection is now automated. `.github/workflows/meshio-watch.yml` runs weekly, compares the declared range against `npm view @meshioplusplus/wasm version`, and files or updates one issue when the latest release falls outside it. It reports rather than acts, because the work a major bump needs (classify, route, re-audit) is exactly what a version comparison and a Dependabot PR cannot do.
...
- **Evidence:** KKSS copies this extension's `dist/meshio` tree beside its application and MCP bundles. In the 2026-09-25 audit, mesh locked **16.14.0** and CAD independently locked 16.7.0; KKSS's bundled mesh/CAD MCP and MMG worker checks passed with the shared tree. Those exercised paths establish a tested baseline, not compatibility with every API or future version.
...
Automate the detection so it does not depend on remembering: **shipped** as `.github/workflows/meshio-watch.yml`, a scheduled job comparing the declared range with `npm view @meshioplusplus/wasm version` and reporting when the latest release is outside it, since that is the one case Dependabot's grouping misses.
...
### 15. Structural CAE mesh formats from meshio++ 16.x — M

**Pending. Opened by the 16.14.0 Tier 0 bump (item 0), which brought eleven structural readers into the WASM build and routed none of them.** They are named individually in `meshCapabilities.ts`'s `UNROUTED_READER_REASONS`, and `mcpTools.test.ts` fails if any of them loses its reason, so this item's inventory is the file — not a comment that can drift.

These are the *geometry* half of what 16.x added, as distinct from the eleven solver-result readers Tier 0 routed. The split is deliberate: a result file is one mesh with a step axis, which the extension's existing timeline machinery already models, while these are input decks from other solvers' preprocessors, each with its own conventions for what a mesh even is.

| Key | Extensions | Note |
|---|---|---|
| `code_aster` | `.mail` | Code_Aster ASCII mesh, 20 element keywords to `HEXA27`; `GROUP_MA`/`GROUP_NO` become cell/point regions. The node ordering is **not** MED's (derived from Code_Aster's gmsh reader). Read/write |
| `elmer` | *(a directory)* | **The one needing real work.** ElmerSolver's mesh directory, recognised by content (`mesh.header`, `partitioning.N`). Needs `openfoamCase.ts`-class staging through `meshCompanionNames`/`collectOpenFoamCase`; a directory cannot be an extension in the Open dialog the way `.foam` is, so the `.foam`-marker precedent applies. Read/write |
| `febio` | `.feb` | FEBio input, spec 2.5/3.0/4.0 read and 4.0 written. Ships `<MeshData>` point/cell data since 16.2.0 |
| `femap` | `.neu` | Femap neutral file, plus 2401 per-value function lists (16.11.0). Read/write |
| `libmesh` | `.xda`, `.xdr` (+`.gz`/`.bz2`) | libMesh 0.7.0→1.8.0 ASCII and big-endian XDR. Edge/shell-face sets are **Breaking** (16.11.0): read as `line` cells in `<name>:edge` regions where they were previously skipped. Read/write |
| `marc` | `.dat`, `.t19` | MSC Marc input deck. **Shares `.dat` with Tecplot**, so it can only be a *retry* candidate, never the default; upstream disambiguates by content, which the extension cannot when it names the format |
| `mfem` | `.mesh` | **Shares `.mesh` with Medit**, same retry-only constraint. Order-3+ `H1` reads as `VTK_LAGRANGE_*` (Breaking, 16.11.0) and a rank file (`.000000`) now reads every sibling rank with `partition:part` — worth deciding deliberately, since that is a hidden multi-file read. Read/write |
| `mphbin` | `.mphbin` | COMSOL binary mesh. Read/write |
| `patran` | `.pat`, `.out` | Patran neutral, now with 9-node quads and 7-node triangles (16.11.0). Read/write |
| `radioss` | `.rad` | OpenRadioss **starter deck — an input, not a result**, so it belongs with these and not with the eleven routed readers. `/BEGIN` length units are applied (Breaking, 16.11.0) and `/SURF` groups become side regions. Read-only |
| `z88` | *(fixed file name)* | Z88 structure file, dispatched by the **literal name** `z88i1.txt`/`z88structure.txt` ahead of `.txt`'s xyz reader — not an extension at all, so it needs `meshExtname`-level handling. Read/write |

**Acceptance:** each routed key has a committed round-trip fixture (writing one is possible for all of them, which is the reason they are tractable — the eleven result formats of item 0 are read-only and admit no such fixture), the retry-only candidates (`.dat`→`marc`, `.mesh`→`mfem`) are proven not to steal the primary format's files, `elmer` stages its directory through the OpenFOAM-class path with a marker, and every `unroutedReaders` entry in `meshCapabilities` shrinks without a key losing its reason. **MCP:** each becomes reachable through the existing `mesh_info`/`mesh_convert` `inputFormat`/`outputFormat` arguments and appears in `mesh_capabilities`; no new tool is required.
...
### 18. VTK-wasm as an experimental alternative renderer — decided

**Decided (2026-09-26): vtk.js stays the renderer; VTK-wasm ships as an experimental, opt-in alternative and will not replace it.** The item began as "replace vtk.js with VTK-wasm" and was worked as a gated migration (the [migration record](./vtk-wasm-migration.md) holds every gate, measurement and evidence file). Phases 0–4 passed: the three blockers of the 2026-09-18 [spike](./vtk-wasm-spike.md) were resolved for the pinned VTK **9.7.20260920** build (they were largely loading artifacts, plus a deterministic glue rewrite that removes Embind's dynamic code), the webview reaches its renderer only through a backend boundary (pixel-identical on vtk.js), the runtime ships verified in the `.vsix` with its licences (+13 MB), and the packaged extension boots VTK-wasm in real VS Code 1.138/1.139 under a CSP that adds only `'wasm-unsafe-eval'` and a scoped `connect-src`, falling back to vtk.js with an explanation when it cannot start. Measured on a software rasterizer, VTK-wasm meets every proposed performance and memory budget. The remaining differences are rendering style rather than defects — thin-line rasterization on dense edge overlays, VTK's own annotation actors, and a coplanar translucent wireframe the order-independent pass drops — and none justifies switching a working default. So the default switch and the vtk.js removal are **not pursued**, and hosts without JSPI keep vtk.js by construction.

**What remains supported:** `kratos.preview.renderer: "vtkwasm"` (tagged experimental) selects the backend for newly opened previews; the boundary (`webview/render/backend.ts`), the asset pipeline (`npm run vtkwasm:prepare`, verified at package time), the upstream watch workflow and the parity/smoke tooling (`scripts/render-parity/`, `scripts/vtk-wasm/g4-*.mjs`) stay, so the alternative keeps working and a future re-evaluation starts from measurements rather than from scratch. Revisit only with a concrete reason vtk.js cannot serve — a feature it lacks, or a real-GPU result (the checklist is in the migration record) showing VTK-wasm materially better.

**MCP:** none. The renderer is UI-only; the headless numerical tools are renderer-independent and keep their contracts.
...
- **Rendering remains a separate runtime concern, and vtk.js remains the renderer.** Software-WebGL translucency, recording with a non-preserved drawing buffer, browser codec availability, and webview CSP restrictions are not fixed by a WASM upgrade. VTK-wasm is available as an experimental opt-in behind the renderer boundary (item 18); switching the default or removing vtk.js is not planned.
