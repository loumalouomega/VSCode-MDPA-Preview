...
| [What's New in VTK-wasm](https://www.kitware.com/whats-new-in-vtk-wasm/) (Kitware, 2026-09: standalone vs remote sessions, `wasm32`/`wasm64` modes, WebGPU/WebXR, `vtk-wasm-sdk` with versioned images and `wasm32`/`wasm64`/`wasm32-testing`/`wasm64-testing` CMake presets) | Tier 0 rendering-runtime spikes: vendored standalone session, wasm64 large-mesh path, WebGPU/effects track — all evaluation-only until a spike lands. |

The historical audits in `src/test/fixtures/transient/README.md` and `CLAUDE.md` remain evidence about the versions tested. During integration, replace obsolete expectations with positive capability tests and retain relevant regression fixtures. Source inspection supports this roadmap; no new live-WASM audit was performed for this documentation change. Reviewed Kitware's [What's New in VTK-wasm](https://www.kitware.com/whats-new-in-vtk-wasm/) on **2026-09-17** as the Tier 0 baseline (standalone vs remote sessions, `wasm32`/`wasm64` modes, WebGPU/WebXR, `vtk-wasm-sdk`); re-verify its claims against a vendored binary during the spikes rather than treating the announcement as API.
...
## Tier 0 — VTK-wasm rendering runtime (evaluation only, vtk.js stays the renderer until a spike lands)

Admission criterion: work that answers whether the real VTK C++ compiled to WebAssembly can replace or complement the current vtk.js reimplementation in the VS Code webview, without regressing picking, split view, capture, or packaging. Nothing here changes the shipped renderer by default; each item is a bounded spike with a keep-or-drop decision.

Key facts from the Tier 0 baseline article: VTK-wasm is the VTK C++ library compiled with Emscripten, not the vtk.js reimplementation; standalone sessions run fully client-side with full VTK class names and a C++/Python-like pipeline model, deployable as static sites from `npm i @kitware/vtk-wasm` with a roughly 12–15 MB runtime-cached binary and a `gen-types` module augmentation keeping `session.vtk` typed to the exact binary in use; `VTK_WRAP_SERIALIZATION` (since VTK 9.6+) generates the marshaling helpers including custom classes; trame's `LocalView` (`pip install trame-vtklocal`) mirrors a server `vtkRenderWindow` by sending geometry/state instead of pixels so the server needs no GPU, with `wasm32`/`wasm64` modes, a `webgpu`/`webgl` rendering option, WebXR support, synchronization progress, `emit_memory` client memory reporting, and `getVtkObject` JS access to live VTK objects; `vtk-wasm-sdk` ships wasm32 plus wasm64 libraries and headers in one versioned Docker image with `wasm32`/`wasm64`/`wasm32-testing`/`wasm64-testing` CMake presets; the upstream roadmap adds data-processing/I-O module coverage to the marshaling infrastructure, dynamic module loading for lighter bundles, and WebGPU ports of volume rendering, ambient occlusion, depth peeling, and shadow maps.

### 0. Standalone-session rendering spike in the webview — M

**Pending.** Boot a throwaway, harness-gated spike that runs a vendored `@kitware/vtk-wasm` standalone session inside the screenshot harness (never the shipped preview until the decision lands): vendor the roughly 12–15 MB binary into `media/` rather than fetching it at runtime (the webview CSP has no CDN allowance and `localResourceRoots` is `media/`-only), generate its types with `gen-types` against the vendored binary so `session.vtk` matches the runtime, and render one committed fixture through vtk.js and vtk-wasm side by side. Compare layer-per-block/SubModelPart actors, field coloring via the shared stop lists, glyph layers (spheres/beams/quiver), clip plane plus cap, cube axes/orientation widget, scalar bar, and the `pickResolve.ts` click-to-probe path (vtk.js `vtkCellPicker` semantics such as the unpopulated `getMapper()` must be re-proven, not assumed). Keep mesh parsing, `MdpaModel`, and all analysis/edit code untouched — only the final polydata-to-pixels stage is under test.

**Acceptance:** the same fixture renders recognizably on both stacks with measured FPS, bundle/`.vsix` delta, and load time; every picking, split-view pane, screenshot/record-capture (`preserveDrawingBuffer: false` synchronous copy), and CSP finding is written down as keeps-working, needs-work, or blocks-adoption. **MCP:** UI-only exemption; any new headless compute reached through the WASM boundary still needs parity.

### 1. wasm64 large-mesh path past the 4 GB ceiling — M

**Pending.** Evaluate the `wasm64` build specifically past the `wasm32` roughly 4 GB addressable-memory ceiling: run the header-summary gate's largest refused fixtures (multi-GB meshes, long transient series) against vendored wasm32 and wasm64 binaries and record peak heap, parse/render latency, and where the 4 GB wall actually bites (geometry buffers vs field arrays vs transient frame accumulation). Mirror the meshio++ loader lesson — threaded/sequential variant selection, name-aware binary resolution, no-`SharedArrayBuffer` fallback, heap release per run — and check what the VS Code webview (including Remote-SSH/WSL software-GL fallbacks) actually permits: `crossOriginIsolated`, threads, and `emit_memory`-style client memory reporting wired into the loading overlay's determinate bar. Decide explicitly whether wasm64 ships always, on demand above a size threshold, or not at all, weighing the extra binary weight against item 19's progressive-preview and selective-field alternatives; pin the SDK version rather than tracking weekly images.

**Acceptance:** a size-vs-memory table naming the largest mesh each variant opens, the failure mode at the ceiling (a clean refusal, never a silent truncation), and the shipping decision with `.vsix` cost. **MCP:** UI-only exemption unless large-mesh reads gain a headless entry point, which then follows item 2's worker/progress contract.

### 2. WebGPU/WebXR and advanced-effects track — S–M, follows item 0

**Pending.** Once the spike renders, measure the `webgpu` backend option on many-actor scenes (outline with dozens of SubModelPart layers plus field/iso/threshold overlays and split-view panes) against forced `webgl`, and track upstream's WebGPU ports (volume rendering, ambient occlusion, depth peeling, shadow maps) plus WebXR only as they land in the pinned release — do not chase weekly SDK tags. Re-verify the two known-fragile paths under each backend: translucent-actor compositing under software WebGL (the existing order-independent-transparency full-opacity caveat) and the synchronous render/copy video/screenshot capture, plus `getVtkObject`-style JS access if the spike needs it for per-pane cameras or clip planes. Dynamic module loading (ship only the VTK libraries used) is the packaging lever if the bundle delta from item 0 is too large.

**Acceptance:** a backend-by-scene matrix (FPS, correctness of transparency/volume/ambient-occlusion where available, capture still byte-valid) against the pinned SDK version. **MCP:** UI-only exemption.
