...
[![VS Code Marketplace](https://img.shields.io/visual-studio-marketplace/v/kratos-multiphysics.vscode-mdpa?label=VS%20Code%20Marketplace)](https://marketplace.visualstudio.com/items?itemName=kratos-multiphysics.vscode-mdpa) [![Installs](https://img.shields.io/visual-studio-marketplace/i/kratos-multiphysics.vscode-mdpa)](https://marketplace.visualstudio.com/items?itemName=kratos-multiphysics.vscode-mdpa) [![GitHub Release](https://img.shields.io/github/v/release/loumalouomega/VSCode-MDPA-Preview)](https://github.com/loumalouomega/VSCode-MDPA-Preview/releases) [![License](https://img.shields.io/badge/license-AGPL--3.0--or--later-blue)](LICENSE) [![Build](https://img.shields.io/github/actions/workflow/status/loumalouomega/VSCode-MDPA-Preview/package.yml?label=build)](https://github.com/loumalouomega/VSCode-MDPA-Preview/actions) [![Documentation](https://img.shields.io/badge/docs-online-brightgreen)](https://loumalouomega.github.io/VSCode-MDPA-Preview/)
...
- **Probe line** (Inspect panel): toggle **Probe line**, click two nodes, and the extension samples a nodal field along the line between them and charts distance-versus-value — the quantitative half of Clip, which answers the same question visually. The line breaks wherever the path leaves the mesh or the field is unwritten, the plot follows the timeline step in mesh series, and CSV export rides the same save dialog as the other panels. Also available headlessly as the `mesh_probe` MCP tool, whose numbers the panel's equal by construction.
...
| Extended (meshio++) | `.msh` (Gmsh), `.inp` (Abaqus), `.bdf`/`.nas`/`.fem` (Nastran), `.unv`, `.mesh` (Medit), `.vol` (Netgen), `.su2`, `.xdmf`/`.xmf`, `.off`, `.dat`/`.tec` (Tecplot), `.avs`, `.f3grid`, `.pf3`, `.mfm`, `.mphtxt` (COMSOL), `.post`/`.dato` (PERMAS), `.ugrid`, `.wkt`, `.xml` (DOLFIN), `.case`/`.geo` (EnSight Gold), `.node`/`.ele` (TetGen), `.poly` (Triangle), `.foam` (OpenFOAM case: polyMesh plus time-directory fields; multi-region and decomposed cases too), `.vtkhdf`/`.hdf`, `.pcd`, `.xyz`/`.xyzn`/`.xyzrgb`, `.k`/`.key`/`.dyn` (LS-DYNA), `.frd` (CalculiX results, read-only) | via [`@meshioplusplus/wasm`](https://www.npmjs.com/package/@meshioplusplus/wasm) 15.4.0. Ambiguous extensions are resolved by content (`.msh` tries Gmsh then ANSYS/FreeFem; `.inp` tries Abaqus then ANSYS). **Gmsh MSH 4.1 needs ≥ 9.7.0** — earlier builds couldn't read a real-world 4.1 file at all (every such file starts with a `$Entities` section the reader used to reject on sight), which is also what carries 4.1's physical-group membership, so upgrading also means 4.1 files now get their named regions as SubModelParts. Since 11.5.0 a `.msh` export also declares `$PhysicalNames` (untagged groups get freshly allocated tags) instead of dropping them. Export also offers write-only SVG/TikZ figures. **OpenFOAM export** (meshio++ ≥ 9.20.0) is the one format that writes a *directory*: picking `.foam` leaves a 0-byte marker there and puts the mesh in `constant/polyMesh/` beside it, with patch names recovered from the model's SubModelParts afterwards (a patch-less mesh still gets the single synthesized `defaultFaces`). **OpenFOAM zones and patch types now survive too**: `cellZones`/`faceZones`/`pointZones` are read as named SubModelParts (alongside the writer's own block-derived zone — the same block-name-survives pattern Exodus and FLAC3D already have), and a recovered patch `type` that needs no extra dictionary key (`wall`/`empty`/`symmetry`/`symmetryPlane`) round-trips instead of always writing `patch`; a type needing extra keys (`cyclic` and similar) is still downgraded, named in the diagnostic. Multi-region and decomposed (`processor*/`) cases are still read as a single `constant/polyMesh` only. **DOLFIN/TetGen/EnSight are now export targets too**: DOLFIN needs at least one triangle/tetrahedron (each field becomes its own `<stem>_<field>.xml` sibling file), TetGen needs 3D points and at least one tetrahedron (`.ele` + a `.node` companion, geometry only), EnSight Gold writes `.case` + a `.geo` companion (geometry only) — an ineligible mesh is refused with the actual reason before the write runs, rather than an upstream error partway through. `.pcd`/`.xyz` have no cell concept, so exporting to either drops all connectivity and keeps only nodes and Nodal fields. **`.f3grid` (FLAC3D) named ZONE/FACE groups become clean SubModelParts** — `zone:Inlet:Default` reads back as plain `Inlet`, and its 2D FACE-section blocks read back as Conditions rather than Elements, since FLAC3D's own file structure separates the two (unlike Gmsh, where dimensions carry no such meaning). **Named groups on a `.msh` export only survive a re-read when they claim no cell any other named group already claims** — Gmsh tags one physical group per element, and every `EntityBlock` already gets its own block-named group covering its full cell set, so an overlapping SubModelPart loses to it; a node-only SubModelPart has no cell to carry a tag on at all and is refused with a named reason rather than silently dropped (`.med`/`.inp` keep point-only groups, if that matters) |
...
The **Selection** toolbar button opens named selection sets over Elements, Conditions and Geometries (each kind its own id space). While the panel is open, **Ctrl+click** toggles the picked entity in the active set, **Box select** turns a left-drag into a rubber-band batch add, and **Lasso** places vertices one click at a time (click the first point or press Enter to close, Escape cancels). A set carries a **seed** — explicit picks, a SubModelPart, a field range (with the same "all/any" rule the viewer's Threshold mode uses), a quality metric (bad / unacceptable elements), or a property id — and **re-resolves it against every new model**: a field seed follows the timeline step by step, explicit picks keep exactly the ids that still exist and prune the rest, so a selection never silently survives onto unrelated ids.

From a set: **New SubModelPart** (an ordinary, undoable operation — the selected cells' nodes ride with them so the Kratos parent/child subset rule holds), **Export** (the selection as its own file, original ids preserved), **Delete entities** (one undoable edit — the complement of the export: conditions on the surviving region stay, constraints vanish with their nodes, fields and SubModelParts follow, orphan nodes are cleaned up), **Isolate / Hide / Restore** (whole block layers, per each button's stated granularity), Frame and Clear.

**Advanced ▸ Properties editor…** lists every `Begin Properties <id>` block: edit any variable in place (Enter commits — number, `True`/`False`, vector, or verbatim text), create/clone/delete sets, and assign a set to a SubModelPart by rewriting its blocks' `propertyIds` rows. Both authoring shapes are exposed: editing a **shared** set in place, or **clone-and-reassign** to point selected blocks at a fresh id. A beam's `CROSS_AREA` resolves through these sets, so an edit re-renders the beam tubes immediately. Headless, `mesh_select` evaluates the same predicates per kind and `mesh_transform` takes `setProperty`/`createProperty`/`cloneProperty`/`assignProperty`/`deleteProperty`/`createSubModelPartFromSelection` (a seed there resolves against the rolling model at apply time) and `deleteEntities` (the selection as one undoable delete). See the documentation site's *Selection & Properties* page for details.
...
Opens the Mesh Size panel — per-node and per-element size statistics, a box-and-whisker plot, and smallest/largest highlighting. See [Mesh Size](https://loumalouomega.github.io/VSCode-MDPA-Preview/guide/mesh-size) above for the full details.
...
Inspect answers "what is the value at a point" (a click) and Plot over time answers "what did one entity do across the run"; the **Probe line** answers the third combination — what the field does along a line, at one step. Toggle **Probe line** next to Measure, click two nodes, and a profile chart opens: distance along the line on the x-axis, the chosen nodal field on the y-axis, one line per component.

The sampling runs in the extension host through one `meshAnalysis` round trip, so the plot's numbers are exactly `mesh_probe`'s for the same endpoints. A sample the mesh cannot answer — the line leaving the domain, or a region the field was never written to — is a break in the line, never a bridged segment, and the panel counts how many samples were uncovered. In a mesh series the plot follows the timeline step: stepping or playing re-samples the same two endpoints on each frame, and a reply that arrives late (a step whose sampling straggles behind the newer one during playback) is dropped rather than allowed to overwrite the newer profile. **CSV** saves the current profile. Works in both previews; in an `.mdpa` tab there is no timeline, so it is a single-frame profile.
...
The extension ships a standalone [MCP](https://modelcontextprotocol.io/) server (`dist/mcpServer.js`) that exposes its mesh and simulation-setup engine to any MCP client (Claude Code, Claude Desktop, …) — no VS Code needed. Build it once with `npm run compile`, then register it, e.g. with Claude Code:
...
| `mesh_select` | Evaluate a **selection predicate** over a mesh and return the entity ids **per kind** (Elements, Conditions, Geometries each keep their own id space): seed by SubModelPart subtree, by a field's `[lo, hi]` window (the viewer's Threshold cells, with the same all/any rule), by a quality metric (bad / unacceptable elements) or by property id — the feed for `mesh_transform`'s `assignProperty` / `createSubModelPartFromSelection` / `deleteEntities`; `outputPath` writes the uncapped ids as JSON |
...
| `mesh_transform` | Apply a sequence of mesh operations (scale/translate/rotate, merge nodes, remove orphans, linear→quadratic, delete/rename SubModelPart, reorganize the SubModelPart tree (create / move / merge / add / remove entities), write mesh-size fields, set/scale the sphere-element `RADIUS`, MMG remesh & level-set split, smooth, reorder, partition, refine, simplexify, linear→linear-only (quadratic→linear), crop, field calculator + nodal/elemental averaging, field gradient/divergence/curl, field Hessian, Zienkiewicz-Zhu error estimate, signed distance to a surface (an imported file, or a SubModelPart already in the mesh), mass-preserving field transfer from another mesh, merge another mesh file, Properties authoring (setProperty / createProperty / cloneProperty / assignProperty / deleteProperty), and selection-driven edits (a SubModelPart built from a selection — explicit per-kind id lists or a seed resolved against the mesh at apply time — or `deleteEntities`, the same ids as one undoable delete)) inline or from a saved Edit-sidebar recipe |
| `mesh_convert` | Convert between formats — ours (`.mdpa`, `.vtk`, `.vtu`, `.vtp`, `.stl`, `.obj`, `.ply`) plus ~45 written by meshio++ (`.msh`, `.inp`, `.bdf`, `.unv`, `.mesh`, `.vol`, `.su2`, `.xdmf`, `.off`, `.poly` (Triangle), `.xml` (DOLFIN), `.ele` (TetGen), `.case` (EnSight Gold), `.vtkhdf`, `.pcd`, `.xyz`, `.k` (LS-DYNA), the HDF5 containers `.cgns`/`.h5m`/`.hmf`/`.med`, plus the field-only `.dex`/`.ip`/`.mff` and write-only `.svg`/`.tikz` figures, …); plus `.e`/`.exo`/`.ex2` (Exodus, lossy — see the format table). `inputFormat`/`outputFormat` override the extension defaults; `timeStep` selects a step of a multi-step input (Exodus, MED, GiD postprocess, CGNS/Tecplot, XDMF, OpenFOAM time directories). `piece`, `dropGhosts` and `region` select the input as for `mesh_info`. Writing `.xdmf` also emits a companion `<stem>.h5`; writing `.xml`/`.ele`/`.case` refuses with a named reason when the mesh has no representable cells (see the format table) |
...
This extension is licensed under the **GNU Affero General Public License, version 3 or later (AGPL-3.0-or-later)** — see [LICENSE](LICENSE). It previously shipped under MIT; the change is required because it now bundles the AGPL-3.0 Flowgraph editor, and an AGPL component makes the combined work AGPL.
