...
**Exodus, GiD postprocess and XDMF** carry their own steps, while **OpenFOAM** lists numeric time directories. These drive the same timeline bar without a filename grammar, and newly appended steps extend the timeline live. The meshio++ 10.20.2 temporal audit found no additional eligible in-file formats; MED supports explicit step selection through MCP but cannot enumerate its steps.
