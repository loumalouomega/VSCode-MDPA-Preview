...
- `extension.ts` — registers two custom editors (`kratos.mdpaPreview` for `.mdpa`, `kratos.vtkPreview` for the mesh formats `.vtk`/`.vtu`/`.vtp`/`.vti`/`.vts`/`.vtr`/`.vtm`/`.stl`/`.obj`/`.ply`) and commands. A shared `postToActive` helper forwards panel-level commands (reset camera, field, etc.) to whichever provider currently has an active panel.
