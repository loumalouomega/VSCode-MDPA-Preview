...
- `extension.ts` — registers the custom editor (`kratos.mdpaPreview`) and four commands.
- `mdpaEditorProvider.ts` — resolves custom editors: parses the document text, serves the webview HTML, and re-parses on change (debounced 250 ms). Tracks the active panel so window-level commands (`resetCamera`, `toggleNodeIds`, `computeQuality`) can reach it.
